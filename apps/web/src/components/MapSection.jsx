...
function MapSection({ zones, allItems, filteredItems, filters, setFilters, resetFilters, ratingVersion }) {
  ...
  }, [filteredItems, ratingVersion, t]);

  return (
    <section id="mapa-section" className="py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-deep">{t('map.interactive')}</p>
          <h2 className="mt-3 font-display text-4xl text-slate-900">{t('map.section_title')}</h2>
          <p className="mt-4 text-lg text-slate-600">{t('map.section_description')}</p>
        </div>
        ...
        <div className="mt-10 grid gap-8 xl:grid-cols-[0.38fr_0.62fr]">
          <aside className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-soft">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-5">
              <div>
                <h3 className="text-xl font-bold text-slate-900">{t('map.zones_title')}</h3>
                <p className="mt-1 text-sm leading-6 text-slate-500">{t('map.zones_hint')}</p>
              </div>
              <button onClick={resetFilters} className="shrink-0 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">{t('map.see_all')}</button>
            </div>
            ...
            <div className="mt-6 rounded-[24px] bg-mist p-5">
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-deep">{t('map.how_title')}</p>
              <ul className="mt-4 space-y-3 text-sm text-slate-700">
                <li className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-gold"></span><span>{t('map.how_1')}</span></li>
                <li className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-gold"></span><span>{t('map.how_2')}</span></li>
                <li className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-gold"></span><span>{t('map.how_3')}</span></li>
              </ul>
            </div>
          </aside>
          ...
          <div className="overflow-hidden rounded-[30px] border border-slate-200 bg-white p-4 shadow-soft">
            <div className="map-header mb-4 flex flex-wrap items-center justify-between gap-3 rounded-[22px] bg-slate-950 px-4 py-3 text-white">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-slate-300">{t('map.header_label')}</p>
                <h3 className="text-lg font-semibold">{t('map.header_title')}</h3>
              </div>
              <div className="flex flex-wrap items-center gap-3 text-xs">
                <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5"><span className="marker-dot marker-dot-lodge"></span>{t('experience.lodges_legend')}</span>
                <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5"><span className="marker-dot marker-dot-guide"></span>{t('experience.guides_legend')}</span>
              </div>
            </div>
