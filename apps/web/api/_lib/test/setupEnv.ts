process.env.NODE_ENV = 'test';
process.env.BSALE_ACCESS_TOKEN = 'test-token';
process.env.BSALE_API_BASE_URL = 'https://api.bsale.io/v1';
process.env.BSALE_OFFICE_ID = '1';
process.env.BSALE_QUOTE_DOCUMENT_TYPE_ID = '7';
process.env.BSALE_PRICE_LIST_ID = '1';
process.env.CORS_ORIGIN = 'http://localhost:5173';
process.env.RESEND_API_KEY = 're_test_key';
process.env.MAIL_FROM = 'Website <noreply@ona.example>';
process.env.ADMIN_EMAIL = 'admin@ona.example';
process.env.RESEND_CONTACT_TEMPLATE_ID = 'contact-form';
process.env.RESEND_QUOTE_TEMPLATE_ID = 'quote-notification';
process.env.RESEND_REVIEW_INVITE_TEMPLATE_ID = 'review-invite';
process.env.RESEND_REVIEW_THANKS_TEMPLATE_ID = 'review-thanks';
process.env.RESEND_REVIEW_ADMIN_TEMPLATE_ID = 'review-admin';
process.env.POSTGRES_URL = 'postgres://user:pass@localhost:5432/ona';
process.env.ADMIN_API_SECRET = 'test-admin-secret-16';
process.env.PUBLIC_APP_URL = 'https://ona.example';
